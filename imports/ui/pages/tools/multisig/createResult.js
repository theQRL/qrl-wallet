...
Template.appMultisigCreationResult.onRendered(() => {
...
Template.appMultisigCreationResult.helpers({
  bech32() {
  ...
  threshold() {
    const threshold = Session.get('txhash').transaction.tx.multi_sig_create.threshold
    return threshold
  },
  signatories() {
    const signatories = Session.get('txhash').transaction.tx.multi_sig_create.signatories
    const weights = Session.get('txhash').transaction.tx.multi_sig_create.weights
  ...
  multiSigAddress() {
    const txnHash = Session.get('transactionHash')
    return binaryToQrlAddress(generateMultiSigAddress(txnHash))
  },
  nodeExplorerUrl() {
...
Template.appMultisigCreationResult.events({
  'click .jsonclick': () => {
    if (!($('.json').html())) {
      setRawDetail()
    }
